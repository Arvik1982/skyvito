import { getCurrentUserAdds, refreshTokens } from '../api'
...
export default function getUserAddsByToken(
  dispatch,
  ...
  refreshTokens(userAssessTokenRedux, userRefreshTokenRedux)
    ...
    .catch((err) => {
      err.access_token
        ? getCurrentUserAdds(err.access_token)
            .then((data) => {
              dispatch(setCurrentUserAdds(data))
            })
            .catch((er) => {
              ...
              dispatch(
                setError('getCurrentUserAdds_2. Перезайдите в приложение'),
              )
            })
        : console.log(err.message)
      dispatch(setError('Сессия истекла. Перезайдите в приложение'))
    })
}
